...
`
	...
	front = strings.ReplaceAll(front, "${sql_params}", addModel(sqlParams,src))
	front = strings.ReplaceAll(front, "${middle}", middleGenerate(src, hasGroup))
	return front
}

// 处理 sql_param 的特殊字符串结构体
// 自动在前面 + model.
// 填充model
func addModel(sqlParams []string,src interface{}) string {
	tmp := ""
	for _, sqlParam := range sqlParams {
		// 遍历 r.ReqSrc,判断sqlParam是否存在
		of := reflect.TypeOf(src)
		for i := 0; i < of.NumField(); i++ {
			if strings.Contains(sqlParam,of.Field(i).Name){
				index := strings.Index(sqlParam, of.Field(i).Name)
				sqlParam = sqlParam[:index] + "model." + sqlParam[index:]
			}
		}
		tmp += "," +sqlParam
	}
...
func middleGenerate(src interface{}, hasGroup bool) string {
	...
	tmp := ""
	for i := 0; i < typeOf.NumField(); i++ {
		if typeOf.Field(i).Name != "StartTime" &&
			typeOf.Field(i).Name != "EndTime" &&
			typeOf.Field(i).Name != "IsDownload" &&
			typeOf.Field(i).Name != "LoginChannel" &&
			typeOf.Field(i).Name != "PageId" &&
			typeOf.Field(i).Name != "PageSize" {
		...
		if i == typeOf.NumField()-1 {
			...
			if hasGroup {
				...
				for j := 0; j < typeOf.NumField(); j++ {
					if typeOf.Field(j).Name != "StartTime" &&
						typeOf.Field(j).Name != "EndTime" &&
						typeOf.Field(j).Name != "IsDownload" &&
						typeOf.Field(j).Name != "LoginChannel" &&
						typeOf.Field(i).Name != "PageId" &&
						typeOf.Field(i).Name != "PageSize" {
...
func openAndWriteStringFile(pathFile string, write string) error {
	...
	if _,e =file.Write([]byte(write));e!=nil{
		return errorx.Wrap(e)
	}
	return nil
}
