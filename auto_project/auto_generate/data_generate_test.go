...
func TestA(t *testing.T) {
	t.Run("测试", func(t *testing.T) {
		...
		needs := map[string]interface{}{
			...
			"path": "/Users/zonst/Desktop/workplace/awesomeProject/goTest/auto_project",
			//渠道汇总sql查询语句和其参数
			"sql_one": []string{"select * from table_name where id=? and name=?", "(GameAreaId*GameId)"},
			//单渠道sql查询语句和其参数
			"sql_two": []string{"select * from table_name where id=? and name=?", "GameAreaId"},
			//下载表格名
			"excel_table_name": "123",
			//日志打印(可以不用管)
			"log_print":"logstat.LogStat",
		}
